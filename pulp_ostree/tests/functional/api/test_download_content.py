...
# Implement tests.functional.utils.get_ostree_content_unit_paths(),
...
class DownloadContentTestCase(unittest.TestCase):
    ...
    def test_all(self):
        ...
        * `Pulp Smash #872 <https://github.com/pulp/pulp-smash/issues/872>`_
        ...
        distributions = DistributionsOstreeApi(client)
        ...
        unit_paths = [choice(paths) for paths in get_ostree_content_paths(repo.to_dict()).values()]
        fixtures_hashes = [
            hashlib.sha256(utils.http_get(urljoin(OSTREE_FIXTURE_URL, unit_path[0]))).hexdigest()
