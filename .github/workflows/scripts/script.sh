...
if [[ "$TEST" = "docs" ]]; then
  ...
  cmd_prefix bash -c "cd pulp_ostree/docs; make diagrams html; tar -cvf docs.tar ./_build"
  docker cp pulp:/pulp_ostree//docs/docs.tar docs.tar
...
./generate.sh pulp_ostree python
cmd_prefix pip3 install /root/pulp-openapi-generator/pulp_ostree-client
sudo rm -rf ./pulp_ostree-client
...
if [[ "$TEST" == "performance" ]]; then
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    cmd_prefix bash -c "pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_ostree.tests.performance"
  else
    cmd_prefix bash -c "pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_ostree.tests.performance.test_$PERFORMANCE_TEST"
  fi
  exit
fi
...
if [ -f $FUNC_TEST_SCRIPT ]; then
  source $FUNC_TEST_SCRIPT
    ...
    if [[ "$GITHUB_WORKFLOW" == "Ostree Nightly CI/CD" ]] || [[ "${RELEASE_WORKFLOW:-false}" == "true" ]]; then
        cmd_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_ostree.tests.functional -m parallel -n 8 --nightly"
        cmd_prefix bash -c "pytest -v -r sx --color=yes --pyargs pulp_ostree.tests.functional -m 'not parallel' --nightly"

    
    else
        cmd_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_ostree.tests.functional -m parallel -n 8"
        cmd_prefix bash -c "pytest -v -r sx --color=yes --pyargs pulp_ostree.tests.functional -m 'not parallel'"
